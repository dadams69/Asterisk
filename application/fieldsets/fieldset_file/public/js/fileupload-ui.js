    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        options: {
            ...
            add: function (e, data) {
                ...
                if(that.options && that.options.onAdd)that.options.onAdd();
            ...
            send: function (e, data) {
                ...
                if (data.context && data.dataType &&
                    ...
                    $(this).find('.fileupload-progressbar .bar').css('width', "100%");
                        ...
                        $(this).fadeOut(function () {
                            that._renderDownload([file])
                                ...
                                .fadeIn(function () {
                                    // Fix for IE7 and lower:
                                    $(this).show();     
                                    if(data.afterDone)data.afterDone();
                                });
                        });
                    });
                } else {
            ...
            progress: function (e, data) {
                if (data.context) {
                    ...
                    data.context.find('.row-progressbar').find('.bar').css(
                        'width',
                        progress + '%'
                    );
                }
            ...
            progressall: function (e, data) {
                ...
                globalProgressNode = $this.find('.fileupload-progressbar');
                globalProgressNode.find('.bar').css(
                        'width',
                        progress + '%'
                    );                
            },
            ...
            start: function () {
                $(this).find('.fileupload-progressbar').fadeIn().find(".bar").css('width', "0");
            },
            // Callback for uploads stop, equivalent to the global ajaxStop event:
            stop: function () {
                $(this).find('.fileupload-progressbar').fadeOut(function(){
        ...
        _renderUpload: function (files) {
            ...
            tmpl.find('.row-progressbar').slice(
                isValidated ? 1 : 0
            ).remove().end().first()
                .fadeIn().find(".bar").css('width', "0");
            //tmpl.find('.start a').slice(
            tmpl.find('a.start').slice(
            ...
            tmpl.find('a.cancel').slice(1).remove().end().first()
            //tmpl.find('.cancel a').slice(1).remove().end().first()
                /*.button({
                    text: false,
        ...
        _initFileUploadButtonBar: function () {
            ...
            this.element.find('.fileinput-button').each(function () {
                ...
                //
                $(this).button(/*{icons: {primary: 'ui-icon-plusthick'}}*/).append(fileInput);
            });
            fileUploadButtonBar.find('.start')
                //.button({icons: {primary: 'ui-icon-circle-arrow-e'}})
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
                    //filesList.find('.start a').click();
                    filesList.find('a.start').click();
                });
            fileUploadButtonBar.find('.cancel')
                //.button({icons: {primary: 'ui-icon-cancel'}})
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
                    filesList.find('a.cancel').click();
            ...
            fileUploadButtonBar.find('.delete')
                ...
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
                    filesList.find('a.delete').click();
                    //filesList.find('.delete a').click();
                });
        },
        ...
        destroy: function () {
            this.element.find('.fileupload-progressbar').hide();
            this.element.find('.fileupload-content')
