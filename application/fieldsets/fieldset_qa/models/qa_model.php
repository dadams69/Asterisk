...
class qa_model extends base_fieldset_model {
    ...
    function array_to_db($fieldset_id,$form){
        $CI = & get_instance();
        $CI->load->database();        
        $CI->db->where('fieldset_qa_id', $fieldset_id);
        $CI->db->update('fieldset_qa', $form);  
    ...
    function db_to_array($fieldset_id){
        ...
        $CI->load->database();
        $CI->db->select('*');
        $CI->db->from('fieldset_qa');
        $CI->db->where('fieldset_qa_id', $fieldset_id);
            
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $fieldset = $query->row();
            $form["fieldset_qa_id"] = get_object_property($fieldset, "fieldset_qa_id");
            $form["version_id"] = get_object_property($fieldset, "version_id");
            $form["question"] = get_object_property($fieldset, "question");
            $form["answer"] = get_object_property($fieldset, "answer");
        }
        return $form;                    
    }
